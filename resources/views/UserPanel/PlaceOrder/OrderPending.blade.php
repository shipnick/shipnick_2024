...
<script>
	document.getElementById('searchForm3').addEventListener('submit', function(e) {
		e.preventDefault();
						...
						<div class="card-body">
							<div class="row">
								...
								<div class="col-lg-12 order-lg-1">
									...
									<form id="searchForm3" action="{{ asset('/UPAll_Pending_Orders') }}" method="post">
										<div class="row">
			...
			<form method="post" action="{{ asset('/filter-selected-order') }}">@csrf
				<div>
					...
					<div class="d-flex justify-content-start align-items-center">
						...
						<button id="pendingExcelBtn" class="btn btn-outline-secondary btn-sm mb-2">
				...
				<div id="newpaginationnew7">
						<!--{{$params->links('pagination::bootstrap-4')}}-->
						{{ $params->withQueryString()->links('pagination::bootstrap-4', ['show_count' => true, 'count_format' => __('Showing :first to :last of :total entries')]) }}
					</div>
				</div>
				<script>
					document.getElementById('pendingExcelBtn').addEventListener('click', function(event) {
						event.preventDefault(); // Prevent the default form submission
						document.getElementById('downloadExcelForm').submit(); // Submit the form manually
					});
				</script>
				<script>
					document.getElementById('pendingExcelBtn').addEventListener('click', function() {
						var cfromdate = "{{ $cfromdate }}";
						var ctodate = "{{ $ctodate }}";
						var ftype = "Excel";

						var url = "{{ asset('/today-pickup-orders') }}";
