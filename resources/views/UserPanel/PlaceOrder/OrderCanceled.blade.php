...
<script>
	document.getElementById('searchForm11').addEventListener('submit', function(e) {
		e.preventDefault();
						...
						<div class="card-body">
							<div class="row">
								...
								<div class="col-lg-12 order-lg-1">
									...
									<form id="searchForm11" action="{{ asset('/UPAll_Canceled_Orders') }}" method="post">
										<div class="row">
			...
			<form method="post" action="{{ asset('/filter-selected-order') }}">@csrf
				<div>
					...
					<div class="d-flex justify-content-start align-items-center">
						...
						<button id="cancelExcelBtn" class="btn btn-outline-secondary btn-sm mb-2">
				...
				<div id="newpaginationnew4">
						{{$params->links('pagination::bootstrap-4')}}
					</div>
				</div>
				<script>
					document.getElementById('cancelExcelBtn').addEventListener('click', function(event) {
						event.preventDefault(); // Prevent the default form submission
						document.getElementById('downloadExcelForm').submit(); // Submit the form manually
					});
				</script>
				<script>
					document.getElementById('cancelExcelBtn').addEventListener('click', function() {
						var cfromdate = "{{ $cfromdate }}";
						var ctodate = "{{ $ctodate }}";
						var ftype = "Excel";

						var url = "{{ asset('/today-cancel-orders') }}";
