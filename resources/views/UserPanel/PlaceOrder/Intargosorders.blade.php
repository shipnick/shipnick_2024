...
<script>
	document.getElementById('searchForm5').addEventListener('submit', function(e) {
		e.preventDefault();
						...
						<div class="card-body">
							<div class="row">
								...
								<div class="col-lg-12 order-lg-1">
									...
									<form id="searchForm5" action="{{ asset('/Transit-orders') }}" method="post">
										<div class="row">
			...
			<form method="post" action="{{ asset('/filter-selected-order') }}">@csrf
				<div>
					...
					<div class="d-flex justify-content-start align-items-center">
						...
						<button id="intransitExcelBtn" class="btn btn-outline-secondary btn-sm mb-2">
				...
				<div class="table-responsive fs-13">
					...
					<div id="newpaginationnew1">
						{{$params->links('pagination::bootstrap-4')}}
					</div>
				</div>
				<script>
					document.getElementById('intransitExcelBtn').addEventListener('click', function(event) {
						event.preventDefault(); // Prevent the default form submission
						document.getElementById('downloadExcelForm').submit(); // Submit the form manually
					});
				</script>
				<script>
					document.getElementById('intransitExcelBtn').addEventListener('click', function() {
						var cfromdate = "{{ $cfromdate }}";
						var ctodate = "{{ $ctodate }}";
						var ftype = "Excel";

						var url = "{{ asset('/today-intransit-orders') }}";
