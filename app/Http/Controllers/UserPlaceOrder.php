...
// Single Orders
    public function SingleOrder(){
        ...
        return view('UserPanel.PlaceOrder.SingleOrder',['params'=>$params,'allriders'=>$allriders,'Hubs'=>$Hubs,'courierapids'=>$courierapids]);
    }
...
if($req->hubid){    $paramsone->where('pickup_id',$req->hubid);    }
...
$paramsone = bulkorders::where('User_Id',$userid)
                        ...
                        ->where('Rec_Time_Date',$tdate);
if($req->mode){    $paramsone->where('Order_Type',$req->mode);          }
if($req->courier){    $paramsone->where('pickup_id',$req->courier);          }
...
foreach($rowData as $key => $value){
    ...
    $custmpincode = trim($value[6]);
    $custmmobile = trim($value[7]);
    $prodname = trim($value[8]);
    $prodqlty = trim($value[9]);
    $paymentmode = strtoupper(trim($value[10]));
    $paymentmode = ($paymentmode == "PP" || $paymentmode == "Prepaid") ? "Prepaid" : "COD";
    $prodcodamt = trim($value[11]);
    $prodweight = trim($value[12]);
    $prodlength = trim($value[13]);
    $prodbreadth = trim($value[14]);
    $prodheight = trim($value[15]);
    $prodinvoiceamt = trim($value[16]);
    $prodtotalamt = trim($value[17]);
    $hub_code = trim($value[18]);
    $errorstatus = "Upload";
    $apistatus = 0;
...
//  -   *   -   *   -   /*  -   /   -   /   -   
    ...
    Http::post('/UPBulk_Order_API');
    Http::post('/UPBulk_Order_API1');
  
   return redirect('/UPBulk_Order_API')->back();
...
foreach($rowData as $key => $value){
  ...
  if($nextstartpoint < $sidno){
    ...
    if($paymentmode == "PP"){           $paymentmode = "Prepaid";    
    }else{        $paymentmode = "COD";    }
...
// Multiple Select Checkbox Order Loop Start
foreach ($selectorders as $id){
// Multiple Select Checkbox Order Loop Start
...
// Multiple Select Checkbox Order Loop End
}
// Multiple Select Checkbox Order Loop End
...
if(empty($current_status_type)){    $current_status_type = "Progress";  }
